...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const resultsContainer = document.querySelector(
        ".results-container .partners-list"
    );
    ...
    travelForm.addEventListener("submit", (event) => {
        ...
        mockData.forEach((mitra) => {
            if (mitra.date === tanggal) {
                ...
                partnerCard.innerHTML = `
                    ...
                    <p class="price">Harga: Rp ${mitra.price.toLocaleString(
                        "id-ID"
                    )}</p>
                    <p class="travel-date">Tanggal: ${new Date(
                        mitra.date
        ...
        if (!resultsContainer.hasChildNodes()) {
            resultsContainer.innerHTML =
                '<p class="no-results">Tidak ada mitra tersedia untuk perjalanan ini.</p>';
        }
    });
});
function pesan(idJadwal) {
    alert("Anda memesan jadwal dengan ID: " + idJadwal);
    // Lakukan tindakan lain seperti redirect ke halaman pemesanan
}
